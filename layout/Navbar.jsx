...
export default function Navbar() {
  ...
  const aboutMenu = [
    ...
    { label: "Familiarization Programme", href: "/about/policy-on-familiarisation-programme" },
  ];
      ...
      <header className="w-full bg-white fixed z-50 ">
        ...
        <nav className="px-4 md:px-14 flex items-center justify-between py-3">
          ...
          <ul className="hidden md:flex items-center gap-8 text-sm font-medium">
            ...
            <li className="relative group cursor-pointer">
              ...
              <div className="absolute left-0 top-full opacity-0 invisible translate-y-2 
           ...
           <Link href={"/careers"}>
            <li className="hover:text-[#43bfb1] cursor-pointer">Careers</li></Link>
          </ul>
        ...
        {open && (
          <div className="md:hidden bg-white border-t px-6 py-4 space-y-4 text-sm font-medium h-screen overflow-y-auto">
            <div className="hover:text-[#43bfb1]"><Link href="/" onClick={() => setOpen(false)}>Home</Link></div>

            {/* ABOUT MOBILE */}
            <div>
              <div onClick={() => setAboutOpen(!aboutOpen)} className="flex justify-between items-center cursor-pointer">
                About Us <ChevronDown size={14} className={aboutOpen ? "rotate-180 transition-transform" : "transition-transform"} />
              </div>
              <div className={`overflow-hidden transition-all duration-300 ${aboutOpen ? "max-h-96 mt-2" : "max-h-0"}`}>
                <div className="pl-4 space-y-3 border-l-2 border-gray-100 ml-1">
                  {aboutMenu.map((item, i) => (
                    <p key={i} className="py-1"><Link href={item.href} onClick={() => setOpen(false)}>{item.label}</Link></p>
                  ))}
                </div>
              </div>
            </div>
            ...
            {/* PRODUCTS MOBILE */}
            <div>
              <div onClick={() => setProductOpen(!productOpen)} className="flex justify-between items-center cursor-pointer">
                Our Products <ChevronDown size={14} className={productOpen ? "rotate-180 transition-transform" : "transition-transform"} />
              </div>
              <div className={`overflow-hidden transition-all duration-300 ${productOpen ? "max-h-96 mt-2" : "max-h-0"}`}>
                <div className="pl-4 space-y-3 border-l-2 border-gray-100 ml-1">
                  {productMenu.map((item, i) => (
                    <p key={i} className="py-1"><Link href={item.href} onClick={() => setOpen(false)}>{item.label}</Link></p>
                  ))}
            ...
            {/* INVESTORS MOBILE */}
            <div>
              <div onClick={() => setInvestorOpen(!investorOpen)} className="flex justify-between items-center cursor-pointer">
                Investors <ChevronDown size={14} className={investorOpen ? "rotate-180 transition-transform" : "transition-transform"} />
              </div>
              <div className={`overflow-hidden transition-all duration-300 ${investorOpen ? "max-h-96 mt-2" : "max-h-0"}`}>
                <div className="pl-4 space-y-3 border-l-2 border-gray-100 ml-1">
                  {investorMenu.map((item, i) => (
                    <p key={i} className="py-1"><Link href={item.href} onClick={() => setOpen(false)}>{item.label}</Link></p>
                  ))}
            ...
            <button className="w-full mt-4 rounded-md bg-[#43bfb1] py-3 text-white">Contact Us</button>
          </div>
        )}
      </header>
    </div>
